...
import com.android.app.orquesta.model.Types;
...
public class UtilityClass {
    ...
    public static void text1ViewScaleIconLeft(Context context, View v, int imageId) {
        TextView conatiner = (TextView) v;
        int imageResource = imageId;
        Drawable drawable = ContextCompat.getDrawable(context, imageResource);
        int pixelDrawableSize = (int) Math.round(conatiner.getLineHeight() * 1); // Or the percentage you like (0.8, 0.9, etc.)
        drawable.setBounds(0, 0, pixelDrawableSize, pixelDrawableSize); // setBounds(int left, int top, int right, int bottom), in this case, drawable is a square image
    ...
    public static String agrupacionToJson(Agrupacion agrupacion) {
        Gson gson = new Gson();
        return gson.toJson(agrupacion);
    }

    public static Agrupacion jsonToAgrupacion(String jsonAgrupacion) {
        Gson gson = new Gson();
        return gson.fromJson(jsonAgrupacion, Agrupacion.class);
    }
